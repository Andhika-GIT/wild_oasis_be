...
func EachDayOfInterval(startDate, endDate string) ([]string, error) {
	var result []string
	var formattedDate string

	// convert StartDate fromn string to time.time
	parsedStartDate, err := time.Parse(time.RFC3339, startDate)

	if err != nil {
		fmt.Println("error when parsing startDate to time : ", err)
		return []string{""}, err
	}

	// convert endDate fromn string to time.time
	parsedEndDate, err := time.Parse(time.RFC3339, endDate)

	if err != nil {
		fmt.Println("error when parsing endDate to time : ", err)
		return []string{""}, err
	}

	for days := parsedStartDate; !days.After(parsedEndDate); days = days.Add(24 * time.Hour) {
		// convert time.Time back to string, with format "Mon Jan 02 2006 15:04:05"
